...
let usernameArgs = {
  where: { username: 'mpita', connected: true }
}
...
test.beforeEach(async () => {
  ...
  AgentStub.findAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.mpita))
...
test.serial('Agent#findByUsername', async t => {
  let agents = await db.Agent.findByUsername('mpita')

  t.true(AgentStub.findAll.called, 'findAll should be called on model')
  t.true(AgentStub.findAll.calledOnce, 'findAll should be called once')
  t.true(AgentStub.findAll.calledWith(usernameArgs), 'findAll should be called with username args')

  t.is(agents.length, agentFixtures.mpita.length, 'agents should be the same amount')
  t.deepEqual(agents, agentFixtures.mpita, 'agents should be the same')
})
